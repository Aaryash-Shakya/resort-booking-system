...
const Payment: React.FC = () => {
  ...
  const onToken = (token: any) => {
    try{
      axios.post(`${serverUrl}/bookings/create/${userId}`, {
        roomId: roomId
      }).then((response) => {
  ...
  return (
    <section className='nav-margin'>
      ...
      <div className="book-now flex justify-around items-center bg-custom-bg-dark rounded-t-xl mt-2 py-2">
        ...
        >
          <button className="btn text-lg rounded-none text-white border-white hover:border-custom-accent bg-custom-accent hover:bg-custom-bg-dark hover:text-custom-accent font-ostwald">Pay Now</button>
        </StripeCheckout>
      </div>
    </section>
  )
